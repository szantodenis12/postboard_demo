...
import { META_APP_ID, PUBLIC_ORIGIN, getLoginUrl, exchangeCodeForToken, fetchUserPages, getEffectiveInstagramAccountId, readConnections, readInstagramOverrides, writeConnections, writeInstagramOverrides, publishToFacebook, publishToInstagram, APP_ORIGIN, } from './meta.js';
import { isAIConfigured, streamChat, streamBrief, streamRewrite, streamHashtags, streamImagePrompt, } from './ai.js';
import { readAnalytics, writeAnalytics, fetchClientAnalytics, readReports, writeReports, } from './insights.js';
import { readWebhooks, writeWebhooks, readWebhookLog, triggerWebhooks, } from './webhooks.js';
import { readSchedulerConfig, writeSchedulerConfig, readSchedulerLog, addSchedulerLog, getBucharestClockTime, getBucharestDateString, isInPublishWindow, } from './scheduler.js';
import crmRouter from './crm.js';
import { GOOGLE_CLIENT_ID, getGoogleLoginUrl, exchangeGoogleCode, fetchLocationInsights, scheduleGoogleInsightsRetry, getGoogleRetryStatus, clearGoogleRetryStatus, processGoogleRetryQueue, readGoogleRetryQueue, isGoogleRateLimitError, fetchGoogleEmail, refreshGoogleConnectionLocations, readGoogleConnections, writeGoogleConnections, readGoogleMapping, writeGoogleMapping, readGoogleAnalytics, writeGoogleAnalytics, publishToGoogle, } from './google.js';
...
async function updatePostCaption(postId, data) {
    await db.collection('posts').doc(postId).update(data);
    notifyChange();
}
async function updatePostDate(postId, date) {
    await db.collection('posts').doc(postId).update({ date });
    notifyChange();
}
async function updatePostPublishOptions(postId, options) {
...
function applyOverrides(data, hiddenIds, options = {}) {
    ...
    // Recalculate totals
    data.totals = {
        posts: data.clients.reduce((s, c) => s + (c.stats.total || 0), 0),
        draft: data.clients.reduce((s, c) => s + (c.stats.draft || 0), 0),
        approved: data.clients.reduce((s, c) => s + (c.stats.approved || 0), 0),
        scheduled: data.clients.reduce((s, c) => s + (c.stats.scheduled || 0), 0),
        published: data.clients.reduce((s, c) => s + (c.stats.published || 0), 0),
    };
    return data;
}
function parseIncludeExpired(value) {
...
function writePageMapping(mapping) {
...
app.get('/api/clients', async (req, res) => {
    try {
        ...
        for (const client of clients) {
            client.posts = allPosts.filter((p) => p.clientId === client.id);
            if (!includeExpired) {
                client.posts = client.posts.filter((post) => post.status === 'published' || post.date >= todayIso);
            }
            client.stats = {
...
app.put('/api/posts/:id', async (req, res) => {
    try {
        ...
        if (requiresInstagramMusic !== undefined)
            updateData.requiresInstagramMusic = requiresInstagramMusic;
...
function inferPostMediaType(value) {
...
function normalizePostMediaStore(raw) {
    const normalized = {};
...
function readPostMedia() {
    ...
    return normalizePostMediaStore(Object.fromEntries(Object.entries(readLegacyPostImages()).map(([postId, image]) => [
        postId,
        [{ ...image, type: 'image' }],
    ])));
}
function writePostMedia(media) {
    const sanitized = {};
    for (const [postId, items] of Object.entries(media || {})) {
        ...
        if (normalizedItems.length > 0)
            sanitized[postId] = normalizedItems;
    }
...
function getPrimaryPostImage(items) {
...
function toLegacyPostImages(media) {
    const images = {};
    for (const [postId, items] of Object.entries(media || {})) {
...
function resolveFacebookPublishMedia(post, items) {
    ...
    if (firstImage)
        return { type: 'image', url: firstImage.url };
    return undefined;
}
function resolveInstagramPublishMedia(post, items) {
    const visualItems = (items || []).filter(item => item.type === 'image' || item.type === 'video');
    const firstVideo = getFirstPostMediaByType(items, 'video');
    const firstImage = getFirstPostMediaByType(items, 'image');
    if (post?.format === 'carousel') {
        if (visualItems.length < 2)
            return { payload: null, error: 'carousel_min' };
        if (visualItems.length > 10)
            return { payload: null, error: 'carousel_max' };
        return {
            payload: {
                type: 'carousel',
    ...
    if (post?.format === 'stories') {
        if (visualItems.length < 1)
            return { payload: null, error: 'story_missing' };
        if (visualItems.length > 1)
            return { payload: null, error: 'story_multiple' };
    ...
    if (post?.format === 'video') {
        if (firstVideo)
            return { payload: { type: 'video', url: firstVideo.url, shareToFeed: true } };
        return { payload: null, error: 'missing_video' };
    }
    if (post?.format === 'reel') {
        if (firstVideo)
            return { payload: { type: 'video', url: firstVideo.url, shareToFeed: false } };
        return { payload: null, error: 'missing_video' };
    }
    if (firstImage)
        return { payload: { type: 'image', url: firstImage.url } };
    return { payload: null, error: 'missing_image' };
}
function appendPostMedia(postId, items) {
...
function replacePostMedia(postId, items) {
    const store = readPostMedia();
    if (items.length > 0)
        store[postId] = items;
    else
...
function removePostMedia(postId, filename) {
    const store = readPostMedia();
    if (!store[postId])
        return [];
    if (!filename) {
    ...
    try {
        ...
        if (media?.type === 'story' && media.media?.url && (media.media?.type === 'image' || media.media?.type === 'video')) {
            ...
            const items = media.items
                ...
                .map((item) => ({ type: item.type, url: String(item.url) }));
...
async function validateReviewToken(token) {
    try {
        ...
        if (!doc.exists)
            return { entry: null, error: 'Invalid or expired review link', status: 404 };
...
app.post('/api/review/:token/posts/:postId/feedback', async (req, res) => {
    ...
    try {
        ...
        if (postDoc.exists) {
            postCaption = postDoc.data().caption?.slice(0, 120);
        }
...
function writeSLAConfig(config) {
...
app.put('/api/sla/config', (req, res) => {
    ...
    if (typeof defaultDeadlineHours === 'number')
        config.defaultDeadlineHours = defaultDeadlineHours;
    if (Array.isArray(reminderIntervals))
        config.reminderIntervals = reminderIntervals;
...
app.get('/api/sla/status', async (_req, res) => {
    try {
        ...
        const config = configDoc.exists ? configDoc.data() : { defaultDeadlineHours: 48, reminderIntervals: [24, 6, 1], clients: {} };
        // 2. Get All Tokens
        ...
        for (const fb of feedback) {
            if (!feedbackByToken[fb.token])
                feedbackByToken[fb.token] = { approved: 0, changesRequested: 0 };
            if (fb.action === 'approved')
                feedbackByToken[fb.token].approved++;
            else
                feedbackByToken[fb.token].changesRequested++;
...
app.post('/api/sla/remind/:token', async (req, res) => {
    ...
    try {
        ...
        const clientName = clientDoc.exists ? clientDoc.data().displayName : entry.clientId;
...
function toGoogleRetryResponse(job) {
    if (!job)
        return null;
    return {
...
// ── Report Routes ────────────────────────────────────────
function normalizeReportPeriods(period, periods) {
...
function formatReportPeriodLabel(periods) {
    if (periods.length === 0)
        return '';
    const sorted = [...periods].sort();
    const format = (value) => new Date(`${value}-01`).toLocaleDateString('ro-RO', { month: 'long', year: 'numeric' });
    return sorted.length === 1
        ? format(sorted[0])
        : `${format(sorted[0])} - ${format(sorted[sorted.length - 1])}`;
}
function getReportPeriodsFromEntry(entry) {
    if (Array.isArray(entry.periods) && entry.periods.length > 0)
        return [...entry.periods].sort();
...
function mergeAnalyticsSnapshots(entries) {
    if (entries.length === 0)
        return null;
    ...
    const sum = (posts, key) => posts.reduce((total, post) => total + post[key], 0);
...
function mergeGoogleSnapshots(entries) {
    if (entries.length === 0)
        return null;
    const dayMap = new Map();
...
async function resolveMetaSnapshotForReport(clientId, periods) {
    const store = readAnalytics();
    const cached = store[clientId] || null;
    if (periods.length === 0)
        return cached;
    if (periods.length === 1 && cached?.period === periods[0])
        return cached;
    const mapping = readPageMapping();
    const pageId = mapping[clientId];
    if (!pageId)
...
async function resolveGoogleSnapshotForReport(clientId, periods) {
    const store = readGoogleAnalytics();
    const cached = store[clientId] || null;
    if (periods.length === 0)
        return cached;
...
app.post('/api/reports/generate', async (req, res) => {
    ...
    try {
        ...
        const effectivePeriods = requestedPeriods.length > 0
            ? requestedPeriods
            : Array.from(new Set([analytics?.period, googleData?.period].filter(Boolean))).sort();
        ...
        for (const post of posts) {
            ...
            try {
                ...
                if (post.platform === 'google') {
                    ...
                    // ── Facebook ──
                    const pageId = mapping[clientId];
                    const page = pageId ? connections.pages.find((p) => p.pageId === pageId) : null;
                    if (!page) {
                        await addSchedulerLog({
                            postId: post.id, clientId, clientName: post.clientName,
                            platform: post.platform, caption: post.caption?.slice(0, 60),
...
async function runAnalyticsSync(options = {}) {
    const config = await readSchedulerConfig();
...
function startSchedulerTimer(intervalMinutes) {
...
function writeWorkflowTasks(tasks) {
...
function getContentFingerprint() {
    ...
    if (now - fingerprintTime < 2000 && cachedFingerprint)
...
function writeUtmClicks(clicks) {
...
function buildUtmUrl(websiteUrl, params) {
