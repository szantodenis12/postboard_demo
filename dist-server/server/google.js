...
export function readGoogleConnections() {
    try {
...
export function writeGoogleMapping(mapping) {
    writeFileSync(GOOGLE_MAPPING_FILE, JSON.stringify(mapping, null, 2), 'utf-8');
}
export function readGoogleAnalytics() {
    try {
...
export function writeGoogleRetryQueue(queue) {
...
async function ensureFreshAccessToken(conn, conns) {
...
async function syncLocationsForConnection(conn, conns) {
...
export async function refreshGoogleConnectionLocations(connectionId) {
    const conns = readGoogleConnections();
...
export async function getValidTokenForLocation(locationName) {
    const conns = readGoogleConnections();
...
const insightsCooldownUntil = new Map();
function getInsightsCooldownKey(locationName, period) {
    return `${locationName}::${period}`;
}
function getRetryJobKey(clientId, period) {
    return `${clientId}::${period}`;
}
function getRetryDelayMs(attempts) {
    return Math.min(GOOGLE_RETRY_MAX_DELAY_MS, GOOGLE_RETRY_BASE_DELAY_MS * Math.max(1, 2 ** attempts));
}
function normalizeInsightsPeriod(period) {
...
function encodeGoogleResourcePath(resourceName) {
    return resourceName
        .split('/')
        .filter(Boolean)
...
async function fetchWithRetry(url, opts, options = {}) {
...
export async function fetchAccounts(token) {
    ...
    const data = await fetchWithRetry(url.toString(), { headers: { Authorization: `Bearer ${token}` } });
    if (data.error)
...
export async function fetchLocations(token, accountName) {
...
export async function fetchLocationInsights(locationName, // "locations/{id}"
period) {
...
export async function publishToGoogle(locationResourceName, summary, imageUrl, ctaUrl) {
    ...
    const res = await fetch(`https://mybusiness.googleapis.com/v4/${locationResourceName}/localPosts`, {
        method: 'POST',
        headers: {
