...
import { buildClientContext, buildClientContextPackage, } from './client-context.js';
// ── Client Context Builder ──────────────────────────────
export function getClientContext(projectRoot, clientId, options = {}) {
    return buildClientContext(projectRoot, clientId, options);
}
export function getClientContextPackage(projectRoot, clientId, options = {}) {
    return buildClientContextPackage(projectRoot, clientId, options);
}
function buildSystemPrompt(clientContext, task, options) {
    const taskLabels = {
        ads: 'paid ads',
...
function runClaude(res, prompt, meta) {
    initSSE(res);
...
function inferChatTask(text) {
    const lower = text.toLowerCase();
    if (/(competitor|competitori|concuren|market|piață|pozitionare|poziționare|strategie)/i.test(lower))
        return 'strategy';
    if (/(website|site|landing page|landing|pagină|pagina)/i.test(lower))
        return 'website';
    if (/(email|newsletter|mail)/i.test(lower))
        return 'email';
    if (/(seo|keyword|blog|articol|organic)/i.test(lower))
        return 'seo';
    if (/(ads|ad copy|reclam|meta ads|google ads|campanie plătită|headline|primary text)/i.test(lower))
        return 'ads';
    if (/(report|raport|analytics|analiz|metric|kpi|engagement|benchmark|performan)/i.test(lower))
        return 'reports';
    if (/(calendar|calendar editorial|plan|luna asta|postare|caption|hashtag|brief)/i.test(lower))
        return 'social-media';
    return 'content-creation';
}
// ── Chat ────────────────────────────────────────────────
export async function streamChat(res, projectRoot, clientId, messages) {
...
// ── Brief Generator ─────────────────────────────────────
export async function streamBrief(res, projectRoot, clientId, platform, format, topic, notes) {
...
// ── Caption Rewriter ────────────────────────────────────
export async function streamRewrite(res, projectRoot, clientId, caption, platform, tone) {
    const context = getClientContext(projectRoot, clientId, {
        task: 'social-media',
...
// ── Hashtag Suggestions ─────────────────────────────────
export async function streamHashtags(res, projectRoot, clientId, topic, platform) {
    const context = getClientContext(projectRoot, clientId, {
        task: 'social-media',
...
export async function streamImagePrompt(res, visualDescription, clientName, platform, format, pillar) {
...
function getAspectRatio(platform, format) {
