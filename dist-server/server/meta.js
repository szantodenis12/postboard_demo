...
export function readConnections() {
    try {
        if (existsSync(CONNECTIONS_FILE)) {
            const connections = JSON.parse(readFileSync(CONNECTIONS_FILE, 'utf-8'));
...
export function readInstagramOverrides() {
    try {
...
const DEFAULT_SCOPES = 'pages_show_list,pages_read_engagement,pages_manage_posts,instagram_basic,instagram_content_publish,business_management';
...
export async function fetchUserPages(userToken) {
    ...
    let url = `https://graph.facebook.com/v21.0/me/accounts?fields=id,name,access_token,instagram_business_account{id},connected_instagram_account{id}&limit=100&access_token=${userToken}`;
    while (url) {
    ...
    try {
        const bizData = await graphFetch(`https://graph.facebook.com/v21.0/me/businesses?fields=id,name&access_token=${userToken}`);
        for (const biz of bizData.data || []) {
            try {
                let bizUrl = `https://graph.facebook.com/v21.0/${biz.id}/owned_pages?fields=id,name,access_token,instagram_business_account{id},connected_instagram_account{id}&limit=100&access_token=${userToken}`;
                while (bizUrl) {
...
async function createInstagramContainer(igAccountId, payload) {
...
async function waitForInstagramContainer(containerId, pageToken, label) {
    const maxAttempts = 20;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const status = await graphFetch(`https://graph.facebook.com/v21.0/${containerId}?fields=status_code,status&access_token=${pageToken}`);
...
async function publishInstagramContainer(igAccountId, pageToken, creationId) {
...
export async function publishToFacebook(pageId, pageToken, message, media) {
    if (media?.type === 'video') {
...
export async function publishToInstagram(igAccountId, pageToken, caption, media) {
    if (media.type === 'story') {
