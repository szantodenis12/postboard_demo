...
function runClaude(res, prompt, options) {
    initSSE(res);
    const env = { ...process.env, NO_COLOR: '1', CLAUDECODE: '' };
    const args = ['-p', '--model', 'sonnet'];
    if (options?.allowWeb)
        args.push('--allowedTools', 'WebSearch,WebFetch', '--permission-mode', 'auto');
...
function resolveClientDir(clientId) {
...
function resolveUniqueFilePath(dir, baseName) {
...
function buildPostMarkdown(post, clientName) {
    ...
    if (post.pillar)
        lines.push(`**Pilon:** ${post.pillar}`);
    if (post.time)
        lines.push(`Ora: ${post.time}`);
    lines.push('');
...
function runClaudeWithParsing(res, prompt, options) {
    initSSE(res);
    const env = { ...process.env, NO_COLOR: '1', CLAUDECODE: '' };
    const args = ['-p', '--model', 'sonnet'];
    if (options?.allowWeb)
...
function parsePostsFromOutput(output) {
    ...
    for (const match of arrayMatches) {
        try {
            ...
            if (Array.isArray(arr) && arr.length > 0 && arr[0].date)
                return validatePosts(arr);
        }
        catch { /* skip */ }
    }
...
router.post('/calendar-fill/apply', (req, res) => {
    ...
    if (!Array.isArray(posts) || posts.length === 0) {
        res.status(400).json({ error: 'Missing or empty posts array' });
        return;
    }
    // Resolve client directory name from clientId
    const clientDir = resolveClientDir(clientId);
    if (!clientDir) {
        res.status(404).json({ error: `Client directory not found for id: ${clientId}` });
        return;
    }
...
All recommendations in Romanian.`;
    ...
    if (!clientId || !Array.isArray(competitors) || competitors.length === 0) {
        res.status(400).json({ error: 'Missing clientId or competitors array' });
        return;
    }
    const context = getClientContext(PROJECT_ROOT, clientId, {
        task: 'strategy',
    ...
    const focusAreas = Array.isArray(focus) && focus.length > 0
        ? focus.map((f) => FOCUS_LABELS[f] || f).join('\n- ')
...
# Comparison Table
| Metric | Our Client | ${competitors.map((c) => c.split('/').pop()?.replace(/^www\./, '') || c).join(' | ')} |
...
router.post('/predict-performance', (req, res) => {
    ...
    if (!clientId) {
        res.json({ error: 'Missing clientId' });
        return;
    }
    ...
    if (a?.combined) {
        ...
        factors.push('No historical data — using industry estimates');
    }
    const len = (caption || '').length;
    if (len > 50 && len < 300) {
        baseEng *= 1.15;
        factors.push('Optimal caption length');
    }
    else if (len > 500) {
        baseEng *= 0.85;
        factors.push('Long caption may reduce engagement');
    }
    const hc = Array.isArray(hashtags) ? hashtags.length : 0;
    if (hc >= 5 && hc <= 15) {
        baseEng *= 1.1;
        factors.push(`${hc} hashtags — good range`);
    }
    else if (hc === 0)
        factors.push('No hashtags — add for reach');
    if (format === 'carousel') {
        baseEng *= 1.25;
        baseReach *= 1.3;
        factors.push('Carousels get +25% engagement');
    }
    else if (format === 'reel') {
        baseReach *= 2;
        factors.push('Reels get 2x reach');
    }
    if (platform === 'instagram') {
        baseEng *= 1.1;
        factors.push('Instagram has higher engagement rates');
    }
    res.json({
    ...
    try {
        ...
        const scores = clients.map(cl => {
            ...
            if (a?.combined?.engagementByDay?.length > 0) {
                ...
                const h1 = d.slice(0, mid).reduce((s, x) => s + x.engagement, 0);
                const h2 = d.slice(mid).reduce((s, x) => s + x.engagement, 0);
            ...
            const contracts = (crm.contracts || []).filter((c) => c.clientId === cl.id);
            let contract = 15;
            if (contracts.some((c) => c.status === 'active'))
                contract = 25;
            else if (contracts.some((c) => c.status === 'expired'))
                contract = 5;
            ...
            const alerts = [];
            if (posting < 10)
                alerts.push('No posts in last 4 weeks');
            if (engagement <= 10)
                alerts.push('Engagement declining');
            if (contract <= 5)
                alerts.push('Contract expired');
            if (approval <= 10)
                alerts.push('Low approval rate');
...
router.get('/best-times', (req, res) => {
    const { clientId } = req.query;
    if (!clientId) {
        res.json({ slots: [], recommendation: 'Select a client' });
        return;
    }
    const store = readJSON(resolve(DATA_DIR, 'analytics.json')) || {};
    const a = store[clientId];
    if (!a?.combined?.engagementByDay?.length) {
        res.json({ slots: [], recommendation: 'No data. Fetch from Meta first.' });
        return;
    }
    ...
    try {
        ...
        const balances = clients.map(cl => {
            ...
            for (const p of cl.posts) {
                ...
                if (pillar)
                    pm.set(pillar, (pm.get(pillar) || 0) + 1);
                else
                    untagged++;
            }
...
router.get('/roi', (req, res) => {
    ...
    try {
        ...
        const crm = readJSON(resolve(DATA_DIR, 'crm.json')) || { contracts: [] };
        const store = readJSON(resolve(DATA_DIR, 'analytics.json')) || {};
        const contracts = (crm.contracts || []).filter((c) => c.clientId === clientId);
...
router.get('/agency-metrics', (_req, res) => {
    try {
        ...
        const now = new Date();
        const mp = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
        const activeContracts = (crm.contracts || []).filter((c) => c.status === 'active');
        const mrr = activeContracts.reduce((s, c) => s + (c.monthlyValue || 0), 0);
        let postsThisMonth = 0, totalPosts = 0;
        const status = { draft: 0, approved: 0, scheduled: 0, published: 0 };
        const pm = new Map();
        const breakdown = cd.clients.map(cl => {
            const mp2 = cl.posts.filter(p => p.date.startsWith(mp)).length;
            postsThisMonth += mp2;
            totalPosts += cl.stats.total;
            status.draft += cl.stats.draft;
            status.approved += cl.stats.approved;
            status.scheduled += cl.stats.scheduled;
            status.published += cl.stats.published;
            for (const p of cl.posts) {
                if (p.pillar) {
            ...
            const c = activeContracts.find((c) => c.clientId === cl.id);
