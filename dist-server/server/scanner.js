...
function parseDate(dateStr, year = 2026) {
    ...
    const textMatch = normalized.match(/(?:luni|marti|miercuri|joi|vineri|sambata|duminica)\s*,?\s*(\d{1,2})\s+(ianuarie|februarie|martie|aprilie|mai|iunie|iulie|august|septembrie|octombrie|noiembrie|decembrie)\s+(\d{4})/) || normalized.match(/(\d{1,2})\s+(ianuarie|februarie|martie|aprilie|mai|iunie|iulie|august|septembrie|octombrie|noiembrie|decembrie)\s+(\d{4})/);
    if (textMatch) {
...
function detectPlatformFromText(text, filePath) {
    const pathLower = filePath.toLowerCase();
    if (pathLower.includes('/instagram/'))
        return 'instagram';
    if (pathLower.includes('/facebook/'))
        return 'facebook';
    if (pathLower.includes('/linkedin/'))
        return 'linkedin';
    if (pathLower.includes('/tiktok/'))
        return 'tiktok';
    if (pathLower.includes('/google/'))
        return 'google';
    const lower = text.toLowerCase();
    if (/\bstories?\b/i.test(lower) && !/post/i.test(lower))
        return 'stories';
    if (/\binstagram\b|\bIG\b/i.test(text))
        return 'instagram';
    if (/\bfacebook\b|\bFB\b/i.test(text))
        return 'facebook';
    if (/\blinkedin\b/i.test(lower))
        return 'linkedin';
    if (/\btiktok\b/i.test(lower))
        return 'tiktok';
    if (/\bgoogle\s*(business|post|profil)/i.test(lower))
        return 'google';
    return 'facebook';
}
function detectFormat(text) {
    if (/\breel\b/i.test(text))
        return 'reel';
    if (/\bcarousel\b|\bcarusel\b/i.test(text))
        return 'carousel';
    if (/\bvideo\b/i.test(text))
        return 'video';
    if (/\bstories?\b/i.test(text))
        return 'stories';
    if (/\btext\b/i.test(text))
        return 'text';
    return 'single-image';
}
function detectStatus(text) {
    if (/publicat|published|✅/i.test(text))
        return 'published';
    if (/programat|scheduled|⏳/i.test(text))
        return 'scheduled';
    if (/aprobat|approved/i.test(text))
        return 'approved';
    return 'draft';
}
/**
 ...
 */
function extractCaption(section) {
    // 1. Code blocks (```...```)
    ...
    const labeledBlock = section.match(/(?:Caption|Copy|Text|Mesaj|Descriere)\s*[:]*\s*\n+((?:>.*\n?)+)/i);
    if (labeledBlock) {
    ...
    for (const bq of blockquotes) {
        ...
        if (text.includes('Dosarul') && text.includes('Cuprins'))
            continue;
        if (text.includes('[README') || text.includes('[←'))
            continue;
...
function findSubsection(subsections, headings) {
    const normalizedHeadings = headings.map(normalizeRomanianText);
    return subsections.find(block => normalizedHeadings.some(prefix => block.normalizedHeading.startsWith(prefix)));
}
function extractTableField(section, labels) {
    for (const label of labels) {
        const pattern = new RegExp(`^\\|\\s*\\*{0,2}${escapeRegExp(label)}\\*{0,2}\\s*\\|\\s*(.+?)\\s*\\|\\s*$`, 'im');
...
function extractSubsectionText(block) {
    if (!block?.content)
        return '';
    const extracted = extractCaption(block.content);
    if (extracted && extracted.length >= 10)
        return stripMarkdown(extracted);
 ...
 */
function extractByPostHeaders(content, filePath, clientId, clientName) {
    const posts = [];
 ...
 */
function extractByStructuredEditorialSections(content, filePath, clientId, clientName) {
    const posts = [];
    ...
    for (const section of postSections) {
        ...
        const buildHashtags = (caption) => [...new Set([...explicitHashtags, ...extractHashtags(caption)])];
        const pushPost = (platform, caption, format = baseFormat) => {
            const cleanedCaption = stripMarkdown(caption);
            if (!cleanedCaption || cleanedCaption.length < 15)
                return;
            posts.push({
                id: `${clientId}-${basename(filePath, '.md')}-${posts.length + 1}`,
                clientId,
 ...
 */
function extractByDayHeaders(content, filePath, clientId, clientName) {
    const posts = [];
 ...
 */
function extractPosts(content, filePath, clientId, clientName) {
...
// ── Main scanner ─────────────────────────────────────────
export function scanClients(rootPath) {
    ...
    clientDirs.forEach((dir, index) => {
        ...
        if (mdFiles.length === 0 && !existsSync(readmePath))
            return;
        ...
        for (const filePath of mdFiles) {
            try {
                ...
                const isRelevant = fileType.includes('social') ||
        ...
        if (allPosts.length === 0 && files.length === 0)
            return;
        // Calculate stats
        const platforms = {
