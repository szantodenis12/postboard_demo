...
function normalizeUseFor(value) {
...
function findMarkdownFiles(dir) {
    ...
    for (const entry of readdirSync(dir)) {
        if (entry.startsWith('.'))
            continue;
        if (entry === 'node_modules' || entry === 'dist' || entry === '_PUBLIC')
            continue;
...
function resolveClientDir(projectRoot, clientId) {
    ...
    for (const entry of readdirSync(clientiDir)) {
        ...
        if (!stats.isDirectory() || entry.startsWith('.') || entry === '_TEMPLATE')
            continue;
...
function readClientDocuments(projectRoot, clientId) {
    const clientDir = resolveClientDir(projectRoot, clientId);
    if (!clientDir)
        return { clientDir: null, docs: [] };
...
function scoreDocument(doc, rule, queryTokens) {
    let score = 0;
    if (doc.isReadme)
        score += 500;
    const priorityScore = {
    ...
    if (doc.useFor.includes('all'))
        score += 30;
    for (const tag of rule.useFor) {
        if (doc.useFor.includes(tag))
            score += 55;
    }
    if (rule.criticalTypes.includes(doc.type))
        score += 70;
...
function selectDocuments(docs, rule, query, options) {
    ...
    for (const entry of scoredDocs) {
        if (fullDocs.length >= fullLimit)
            break;
        if (entry.doc.isReadme)
            continue;
        if (entry.score < 70)
            continue;
    ...
    for (const entry of scoredDocs) {
        if (referenceDocs.length >= referenceLimit)
            break;
        if (entry.score < 20)
            continue;
        if (fullDocs.some(doc => doc.path === entry.doc.path))
            continue;
...
export function buildClientContextPackage(projectRoot, clientId, options = {}) {
    ...
    if (includeRecentPosts && client?.posts?.length) {
        ...
        for (const post of recent) {
            ...
            if (post.hashtags.length > 0)
                parts.push(`  Hashtags: ${post.hashtags.map(tag => `#${tag}`).join(' ')}`);
        }
        parts.push('');
    }
    return {
...
export function buildClientContext(projectRoot, clientId, options = {}) {
